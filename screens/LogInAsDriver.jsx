...
import { firebaseAuth, firebaseAuth3 } from "../Firebase";
...
const LogInAsDriver = ({navigation}) => {
  ...
  const { passwordVisibility, rightIcon, handlePasswordVisibility } =
    ...
    const handleSubmit = (e) => {
      e.preventDefault();
      setDisiabled(true)
      signInWithEmailAndPassword(firebaseAuth3, email, password)
      .then((userCredential) => {
        // Signed in
        const user = userCredential.user;
        setTimeout(() => {
          navigation.navigate('Driver Dash', {driver_email:email});
          setDisiabled(false);
        }, 3000);
        // ...
        setErrorMes(false);
    ...
    <ScrollView style={{ }}>
      ...
      <View
        ...
        <KeyboardAvoidingView
          behavior="padding"
          style={{ flex: 1, gap: 20, margin: 20, marginTop: portrait ? 40 : 0 }}
        >
          <TextInput
            label="Email"
            leading={(props) => (
          ...
          />
          <Button tintColor="#B76D68" style={{margin:40,marginTop:20,backgroundColor:'#2C2B3C' ,marginBottom:0}} disabled={disabled} onPress={handleSubmit} title="Log In" />
